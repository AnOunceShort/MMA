...
def create_visualization(data, output_file='wallet_visualizer_memesai.html'):
    ...
    for wallet, tickers in wallet_ticker_map.items():
        ...
        for ticker in tickers:
            ticker_id = f"{wallet}_{ticker}"  # Create unique ID for each wallet-ticker combination
            is_memesai = ticker.lower() == 'memesai'  # Check if it's a MemesAI ticker
            nodes.append({
                'id': ticker_id,
                'label': ticker,
                'size': 25 if is_memesai else 20,  # Make MemesAI nodes 25% larger
                'profit': ticker_profit_map[ticker_id],
                'color': '#ff0000' if is_memesai else '#808080'  # Already making MemesAI red
            })
    ...
    node_trace = go.Scatter(
        ...
        hoverinfo='text',
        text=node_text,
        customdata=[node['id'] for node in nodes],  # Add customdata for identifying nodes
        marker=dict(
            size=node_size,
            color=node_color,
            line=dict(
